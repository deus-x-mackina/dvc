...
let managedPath: String = ProcessInfo.processInfo.environment["DVC_ROOT"] ??
                          URL(fileURLWithPath: ProcessInfo.processInfo.environment["HOME"]!)
                              .appendingPathComponent(defaultManagedName)
                              .path
...
func configure() {
    ...
    if CommandLine.argc == 1 ||
       CommandLine.arguments.contains(where: { s in s.contains("help")}) ||
    ...
    guard FileManager.default.fileExists(atPath: gitPath),
          FileManager.default.isExecutableFile(atPath: gitPath) else
    {
        quit("Cannot find a usable git executable; tried: \(gitPath)")
    }
...
fileprivate func whichGit() -> String {
    ...
    let output = String(rawOutput.prefix(through: rawOutput.index(rawOutput.endIndex, offsetBy: -2)))
    return output
}
